...
public class TestPhysicsRayCast extends SimpleApplication
{
	...
	{
		List<PhysicsRayTestResult> rayTest = bulletAppState.getPhysicsSpace()
				.rayTest(cam.getLocation(),
						cam.getLocation().add(cam.getDirection()));
		if (rayTest.size() > 0)
		{
	...
	private void initCrossHair()
	{
		...
		bitmapText.setLocalTranslation(
				(settings.getWidth() - bitmapText.getLineWidth()) * 0.5f,
				(settings.getHeight() + bitmapText.getLineHeight()) * 0.5f, 0);
