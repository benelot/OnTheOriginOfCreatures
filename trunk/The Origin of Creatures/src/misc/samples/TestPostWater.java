...
public class TestPostWater extends SimpleApplication
{
	...
	public void simpleInitApp()
	{
		...
		// cam.setRotation(new Quaternion().fromAngleAxis(0.5f,
		// Vector3f.UNIT_Z));
		...
		// fpp.addFilter(water);
		/*
		 * BloomFilter bloom = new BloomFilter(); //bloom.getE
		 * bloom.setExposurePower(55); bloom.setBloomIntensity(1.0f);
		 * fpp.addFilter(bloom); LightScatteringFilter lsf = new
		 * LightScatteringFilter(lightDir.mult(-300));
		 * lsf.setLightDensity(1.0f); fpp.addFilter(lsf);
		 */
		...
		waves = new AudioNode(assetManager,
				"Sound/Environment/Ocean Waves.ogg", false);
		...
		/*
		 * inputManager.addListener(new ActionListener() { public void
		 * onAction(String name, boolean isPressed, float tpf) { if (isPressed)
		 * { if (name.equals("foam1")) { water.setFoamTexture((Texture2D)
		 * assetManager.loadTexture("Common/MatDefs/Water/Textures/foam.jpg"));
		 * } if (name.equals("foam2")) { water.setFoamTexture((Texture2D)
		 * assetManager.loadTexture("Common/MatDefs/Water/Textures/foam2.jpg"));
		 * } if (name.equals("foam3")) { water.setFoamTexture((Texture2D)
		 * assetManager.loadTexture("Common/MatDefs/Water/Textures/foam3.jpg"));
		 * }
		 * 
		 * if (name.equals("upRM")) {
		 * water.setReflectionMapSize(Math.min(water.getReflectionMapSize() * 2,
		 * 4096)); System.out.println("Reflection map size : " +
		 * water.getReflectionMapSize()); } if (name.equals("downRM")) {
		 * water.setReflectionMapSize(Math.max(water.getReflectionMapSize() / 2,
		 * 32)); System.out.println("Reflection map size : " +
		 * water.getReflectionMapSize()); } } } }, "foam1", "foam2", "foam3",
		 * "upRM", "downRM");
		 */
	...
	public void simpleUpdate(float tpf)
	{
		...
		uw = true;
		/*
		 * } if (!water.isUnderWater() && uw) { uw = false;
		 * //waves.setReverbEnabled(false); waves.setDryFilter(new
		 * LowPassFilter(1, 1f)); //waves.setDryFilter(new LowPassFilter(1,1f));
		 * 
		 * }
		 */
	}
