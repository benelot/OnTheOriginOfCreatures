...
public class TestSweepTest extends SimpleApplication
{
	...
	private CapsuleCollisionShape obstacleCollisionShape = new CapsuleCollisionShape(
			0.3f, 0.5f);
	private CapsuleCollisionShape capsuleCollisionShape = new CapsuleCollisionShape(
			1f, 1f);
	...
	public void simpleInitApp()
	{
		...
		obstacle.move(2, 0, 0);
		RigidBodyControl bodyControl = new RigidBodyControl(
				obstacleCollisionShape, 0);
	...
	public void simpleUpdate(float tpf)
	{
		...
		List<PhysicsSweepTestResult> sweepTest = bulletAppState
				.getPhysicsSpace().sweepTest(
						capsuleCollisionShape,
						new Transform(capsule.getWorldTranslation()),
						new Transform(capsule.getWorldTranslation().add(dist,
								0, 0)));
		...
		if (sweepTest.size() > 0)
		{
			...
			fpsText.setText("Almost colliding with "
					+ collisionObject.getUserObject().toString());
		} else
		{
